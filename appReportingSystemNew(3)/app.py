    ...
    if request.method == 'POST':
        ...
        'G1', 'G2', 'H1', 'H2', 'J1', 'J2', 'K1', 'K2', 'L1', 'L2', 'M1', 'M2', 'N1', 'N2']:
            if request.form.get(posisiFooter) is not None:
                if posisi_footer == '':
                    posisi_footer += request.form.get(posisiFooter)
                    posisi_footer.append(request.form.get(posisiFooter))
                    posisiFooter1 = json.dumps(posisi_footer)
                else:
                    posisi_footer += ", "+request.form.get(posisiFooter)
                    posisi_footer.append(request.form.get(posisiFooter))
                    posisiFooter1 = json.dumps(posisi_footer)
        ...
        requests.post('http://127.0.0.1:5002/saveFormatTemplate/'+detailKolom1+'/'+kolomTemp1+
            '/'+posisiTemp1+'/'+tipeTemp1+'/'+lebarTemp1+'/'+kolomFooter1+'/'+posisiFooter1)
