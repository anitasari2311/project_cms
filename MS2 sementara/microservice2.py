...
#         return view_schedule
    ...
    def editSchedule(self, kode_laporan, header, keterangan, note, jadwalBln, jadwalHari, jadwalTgl, reportPIC, reportPenerima, 
        ...
        # val = kode_laporan, organisasi, server, kategori, header, keterangan, note, penerima, grouping, orderby, jadwal
            ...
            db.commit()
            cursor.execute( 'UPDATE t_schedule SET sch_groupBy = "'+grouping+'", sch_hari = "'+jadwalHari+'", sch_tanggal= "'+jadwalTgl+'", sch_bulan= "'+jadwalBln+'", sch_reportPIC= "'+reportPIC+'", sch_penerima= "'+reportPenerima+'", sch_note="'+note+'", sch_lastUpdate = "'+str(lastUpdate)+'" WHERE report_id = "'+kode_laporan+'"')
            db.commit()
