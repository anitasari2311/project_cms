...
class RequestLaporan:
    ...
    def __init__(self):
        ...
        self.req_judul = ''
        self.req_deskripsi = ''
        self.req_tujuan =          self.req_tampilan = ''
        self.req_periode = ''
        self.req_deadline = ''
        self.req_file = ''
        self.req_PIC = ''
...
#===========================[ TEMPLATE LAPORAN ] ======================================
    ...
    def getReportID():
        try: 
            ...
            cursor.execute('select report_id, report_judul from m_report')
    ...
    @app.route('/getJudulLaporan/<kode_laporan>', methods = ['GET', 'POST'])
    def getJudulLaporan(kode_laporan):
        try: 

            db = databaseCMS.db_template()
            cursor = db.cursor()
     
            cursor.execute('select report_judul from m_report where report_id = "'+kode_laporan+'" ')
            
            listJudulReport = cursor.fetchall()

            listJudul = []

            for row in listJudulReport:
                listDict = {
                'ReportJudul' : row[0]
                }
                listJudul.append(listDict)

            resultListJudul = json.dumps(listJudul)

            print("=== [ getReportJudul ] ===")
            print("==========================")

            return resultListJudul
