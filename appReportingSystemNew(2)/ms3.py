...
def testPreviewLaporan(kode_laporan):
    ...
    if jmlHead == '1':
        try:
            ...
            formatTipeKolom = workbook.add_format()
            formatTipeKolom.set_font_name('Times New Roman')
            # for k in range(len(formatKolom)):
                
            #     if formatKolom[k] == 'Integer':
            #         formatTipeKolom.set_num_format('0.000')

            #     elif formatKolom[k] == 'Decimal':
            #         formatTipeKolom.set_num_format('#,##0.00')
            #     elif formatKolom[k] == 'Percentage':
            #         formatTipeKolom.set_num_format(9)
            #     print(formatKolom[k])
            #     # elif formatKolom[k] == 'Date':
            #     # elif formatKolom[k] == 'Time':
            #     # elif formatKolom[k] == 'Datetime':
                
                    

            #     worksheet.write_row(row2+8,kol+m,'a',formatTipeKolom)
            #     row2=row2+1
            ...
            for i in range(lengthOfData2): #untuk menulis data
                for k in range(len(formatKolom)):
                    if formatKolom[k] == 'Integer':
                        ...
                        formatTipeKolom.set_num_format(9)
                    print(formatTipeKolom)
                worksheet.write_row(row2+8,kol+m,data[i],formatTipeKolom)
                row2 = row2 + 1
