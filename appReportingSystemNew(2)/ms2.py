...
class Template:
    ...
    def detailFormatTemplate(kode_laporan):
        try: 
            ...
            cursor.execute(' SELECT c.report_id, c.report_periode, c.report_printAllYN, c.report_judul, c.report_header, c.report_footer, c.report_jumlahTampilan, a.format_merge, a.format_rataTengah, a.format_rataKanan FROM m_report c LEFT JOIN m_detailh a ON c.report_id = a.report_id WHERE a.report_id = "'+kode_laporan+'" ')
    ...
    @app.route('/namaKolom/<kode_laporan>', methods = ['POST', 'GET'])
    def namaKolom(kode_laporan):
        try:
            db = databaseCMS.db_template()
            cursor = db.cursor()

            cursor.execute(' SELECT nama_kolom FROM m_detailh WHERE report_id = "'+kode_laporan+'" ')

            namaKolom = cursor.fetchall()

            if(len(namaKolom) != 14):
                for i in range(len(namaKolom), 14):
                    namaKolom.append("")
            dumpE = json.dumps(namaKolom)
    ...
    @app.route('/posisiKolom/<kode_laporan>', methods = ['POST', 'GET'])
    def posisiKolom(kode_laporan):
        try:
            db = databaseCMS.db_template()
            cursor = db.cursor()

            cursor.execute(' SELECT lokasi FROM m_detailh WHERE report_id = "'+kode_laporan+'" ')

            posisiKolom = cursor.fetchall()

            if(len(posisiKolom) != 14):
                for i in range(len(posisiKolom), 14):
                    posisiKolom.append("")
            dumpE = json.dumps(posisiKolom)

            print(len(posisiKolom))
            print(posisiKolom)
            return dumpE
    ...
    @app.route('/tipeKolom/<kode_laporan>', methods = ['POST', 'GET'])
    def tipeKolom(kode_laporan):
        try:
            db = databaseCMS.db_template()
            cursor = db.cursor()

            cursor.execute(' SELECT format_kolom FROM m_detailh WHERE report_id = "'+kode_laporan+'" ')

            tipeKolom = cursor.fetchall()

            if(len(tipeKolom) != 14):
                for i in range(len(tipeKolom), 14):
                    tipeKolom.append("")
            dumpE = json.dumps(tipeKolom)
    ...
    @app.route('/lebarKolom/<kode_laporan>', methods = ['POST', 'GET'])
    def lebarKolom(kode_laporan):
        try:
            db = databaseCMS.db_template()
            cursor = db.cursor()

            cursor.execute(' SELECT lebar_kolom FROM m_detailh WHERE report_id = "'+kode_laporan+'" ')

            lebarKolom = cursor.fetchall()

            if(len(lebarKolom) != 14):
                for i in range(len(lebarKolom), 14):
                    lebarKolom.append("")
            dumpE = json.dumps(lebarKolom)

            print(len(lebarKolom))
            print(lebarKolom)
            return dumpE
