...
def sendFormatTemplate():
    ...
    if request.method == 'POST':
        ...
        for lokasiFooter in ['B1', 'C1', 'D1', 'E1', 'F1', 'G1', 'H1', 'I1', 'J1', 'K1', 'L1', 'M1', 'N1']:
            if request.form.get(lokasiFooter) is not None:
                if lokasiFooter == '':
                    lokasiFooter += request.form.get(lokasiFooter)
                    posisiFooter.append(request.form.get[lokasiFooter])
                    posisiFooter1 = json.dumps(posisiFooter)
                else:
                    lokasiFooter += ", "+request.form.get(lokasiFooter)
                    posisiFooter.append(request.form.get[lokasiFooter])
                    posisiFooter1 = json.dumps(posisiFooter)
        ...
        requests.post('http://127.0.0.1:5002/saveFormatTemplate/'+detailKolom1+'/'+kolomTemp1+
            '/'+posisiTemp1+'/'+tipeTemp1+'/'+lebarTemp1+'/'+kolomFooter1)
...
def formatTemplate():
    ...
    if request.method == 'POST':
        ...
        nKolom = requests.get('http://127.0.0.1:5002/namaKolom/'+kode_laporan)
        nKolomDump = json.dumps(nKolom.json())
        loadnKolom = json.loads(nKolomDump)

        pKolom = requests.get('http://127.0.0.1:5002/posisiKolom/'+kode_laporan)
        pKolomDump = json.dumps(pKolom.json())
        loadpKolom = json.loads(pKolomDump)

        tKolom = requests.get('http://127.0.0.1:5002/tipeKolom/'+kode_laporan)
        tKolomDump = json.dumps(tKolom.json())
        loadtKolom = json.loads(tKolomDump)

        lKolom = requests.get('http://127.0.0.1:5002/lebarKolom/'+kode_laporan)
        lKolomDump = json.dumps(lKolom.json())
        loadlKolom = json.loads(lKolomDump)
        ...
        return render_template('testFormatTemplate.html', kode_laporan=kode_laporan
            ...
            namaKolom = loadnKolom, posisiKolom = loadpKolom, tipeKolom = loadtKolom,
            lebarKolom = loadlKolom)
